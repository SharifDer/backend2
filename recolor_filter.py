...
def find_cardinal_extreme_points(coordinates: List[Dict[str, float]]) -> Dict[str, Dict[str, float]]:
...
async def calculate_regional_driving_speed(cardinal_extremes: Dict[str, Dict[str, float]]) -> float:
   """
   ...
   if tasks:
       ...
       for i, result in enumerate(results):
           ...
           try:
               if not isinstance(result, Exception) and result.route and result.route[0].static_duration:
                   drive_time_seconds = int(result.route[0].static_duration.replace("s", ""))
                   distance_meters = calculate_distance(point1, point2)
                   
                   if drive_time_seconds > 0:
                       speed_mps = distance_meters / drive_time_seconds  # meters per second
